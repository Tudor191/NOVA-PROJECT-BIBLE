"""Structural smoke test for the coding-agent Agent Package (doc 12 §3/§4)
...
performs at Manifest Validation and Sandbox Test Run, TDD 3E §5). Mirrors
`agents/research-agent/tests/test_agent_package.py` exactly.
"""
...
def test_agent_yaml_validates_against_agent_manifest() -> None:
    ...
    assert manifest.id == "coding-agent"
    assert manifest.category == "coding"
    assert "inprocess" in manifest.supported_execution_backends
    assert manifest.peer_reviewer_category == "architect"
    assert set(manifest.required_capabilities) == {"git", "filesystem", "terminal"}
