"""Structural smoke test for the architect-agent Agent Package (doc 12 §3/§4)
...
`agents/qa-agent/tests/test_agent_package.py` exactly.
"""
...
def test_agent_yaml_validates_against_agent_manifest() -> None:
    ...
    assert manifest.id == "architect-agent"
    assert manifest.category == "architect"
