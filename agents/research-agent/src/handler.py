...
model_gateway=..., action_port=...)` as keyword-only, defaulted arguments --
all three default to `None` so `agent-os/registry`'s own install-time On
Load smoke test (`handler_class()`, zero arguments) still succeeds;
`execute()` is the one method that actually needs `model_gateway`
populated, and it is always populated by the time a real Kernel dispatch
calls it. `action_port` is accepted here (coding-agent slice, disclosed
addition to this already-established convention) but never used by this
agent's own `execute()` -- `InprocessExecutionBackend` passes the same
three kwargs uniformly to every Handler class it constructs rather than
special-casing per agent category. See
