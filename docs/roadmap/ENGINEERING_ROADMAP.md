...
- **3B — Planning Persistence** (Postgres persistence, `GET`/`POST /v1/plans` API, `planning.task_graph.created`/`planning.decompose.request`, branch `phase-3b-planning-persistence`) — Complete, locally fully verified; PR not yet merged, real GitHub Actions CI pending. This closes TDD 3B's approved scope entirely except §6.1's `agent_os.task.completed` subscription, correctly deferred to Phase 3E. [Gate Review](architecture-reviews/phase-3b-planning-persistence-gate-review.md).
