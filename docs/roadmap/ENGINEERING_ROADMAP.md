...
> | **4C** | Containerize `agent-os`; `GET /v1/agents`; Agents panel | **Implemented, not merged.** *(This cell read "Not started" until 2026-09-11; it was stale and is corrected here.)* **4C.1** containerized `agent-os` (Dockerfiles, `build-and-scan` matrix entries, compose services). **4C.2** built the Agents surface end to end in six slices on `phase-4c.2` — the internal Registry `list_packages` RPC, the append-only `agent_os.agent_activity` table, the Kernel's read-only `GET /v1/agents` fronted by `api-gateway`, activity writes wired into the real Kernel lifecycle, `agent_os.task.completed` opened to the browser through `ws-gateway`, and the Agents panel, route and realtime reconciliation. **PR [#26](https://github.com/Tudor191/NOVA-PROJECT-BIBLE/pull/26) (`phase-4c.2` → `phase-4`) was MERGED 2026-09-12** as the two-parent merge commit `b1d7ca5` (not squashed), on explicit user authorization under CONDITIONAL-GO because C-3 is an approved deferral; 34 of 34 CI check runs were green against head `7360763`, including all 12 `real-infra` jobs. *(This sentence read "is open and NOT merged … against head `1182816`" until the merge; corrected here.)* **Phase 4C is exactly 4C.1 and 4C.2 — there is no 4C.3/4C.4/4C.5/4C.6, and 4C implementation is complete.** Health record: [`phase-4c.md`](../project-health/phase-4c.md). **Gate verdict: CONDITIONAL-GO** — conditions C-1 (real-infrastructure suites) and C-2 (no CI against the head) are **discharged**; **C-3 remains open: AC-4 is still NOT MET**, its "renders live agent instances" and "at least one real peer-review round" clauses **Deferred by explicit user approval of 2026-09-07** because `agent_instance` has exactly one creation path and it is LLM-dependent with no deterministic fallback (master scope §1.1). Real-Postgres CI exposed two defects during verification — a transaction-ordering fault in the Kernel repository and a test-isolation fault in the Phase 3E E2E — both fixed, with decision D-3 and `list_instances()` semantics unchanged. `docs/project-health/phase-4c.md` is not yet written and is carried as a deferred obligation of Phase 4C closure. See the [Gate Review](architecture-reviews/phase-4c2-agents-surface-gate-review.md) §15 and §18 |
> | **4D** | `autonomy-engine` Levels 0–1 + Autonomy panel | **COMPLETE, MERGED AND CLOSED — gate verdict GO** (2026-09-14; *read "CONDITIONAL-GO" through 2026-09-13, when C-4 and C-5 were still open*). Merged into `phase-4` on explicit user authorization as **`68397a200f08db497fc4f36a1b14422dc953298e`**, a normal two-parent merge of PR #27 at head `dff646c`; `phase-4d` is preserved at `dff646c`, not deleted, and is an ancestor of `phase-4`. `main` is untouched at `7e273e6`. **All five conditions C-1…C-5 are DISCHARGED** and CI is **36/36 green at head `9afddf6`**, zero failed and zero cancelled. *(That sentence read "**Still not merged** — PR #27 is open and merge authorization is the user's" until the 2026-09-14 closure.)* **C-5** was fixed at its root by PR #28 (the stale `tools/scaffold-engine.py` Dockerfile template), merged into `phase-4` as `dd6a147` and inherited here; **C-4** was discharged by a ratified documentation-only reconciliation of TDD §5.3 and §4.1. **Three carry-forwards stay OPEN — CF-9, CF-10, CF-11** — each disclosed, each outside 4D's ratified boundary, none affecting an acceptance criterion. ***The remainder of this row is the superseded 2026-09-13 record, preserved per protocol §0.3.4 and true at head `f5f263f`; where it says C-4 and C-5 are open, that `phase-4` is untouched, or that the verdict must not be read as GO, it is superseded by the paragraph above.*** Built on `phase-4d`, head `de6dbc9`, in four slices: the domain layer (Autonomy Levels, Policy Engine, Permission Matrix, Trust Engine, decision pipeline), the `autonomy` Postgres schema with doc 07's append-only `decision_log`, the `/v1/autonomy/*` REST surface fronted 1:1 by `api-gateway`, and the `autonomy/` panel. **Not merged.** PR **#27** was opened on 2026-09-13 **solely to obtain CI evidence**, with merge authorization explicitly withheld; head is now `f5f263f`. **36 checks, 28 success, and every check exercising 4D green**: `checks`, `real-infra (autonomy-engine)` **16/16 on two consecutive heads**, and `Playwright` with **both AC-5 specs passed in a real browser** against the real stack. **C-1, C-2 and C-3 are DISCHARGED.** **Two conditions remain open, neither an approved deferral**, so this verdict must still not be read as GO: **C-4** two TDD clauses need reconciling, and **C-5 (new)** `build-and-scan (ws-gateway)` fails Trivy on 3 CRITICAL + 9 HIGH pre-existing Debian CVEs — `git diff origin/phase-4..HEAD -- services/ws-gateway/` is **0 lines**, so it is not this milestone's, and fixing it is outside this branch's ratified scope, so it is reported rather than fixed. Eleven of twelve acceptance criteria fully Met; **AC-5 now Met at every tier including the browser**. All twelve of TDD §16's negative controls were demonstrated by removing the property and observing the suite fail. **Two ratified decisions govern the scope:** **D-4D-1** — 4D claims no `autonomy.*` Event Bus subject, both allow-lists stay empty, `PUBLIC_TOPICS` is byte-identical, and the AC-5 lifecycle runs over REST and the UI only; **D-4D-2** — **CF-9 stays OPEN**, `action-engine` keeps sole ownership of `IdentityConfidencePolicy`, and its fail-closed defaults are untouched. **Level 2 is defined and disabled** per decision D-1, which assigns enabling it to 4F. **Two new carry-forwards**: **CF-10** no read surface exists for 2D-D's `TrustMetric`, so TDD §5.3's Event Bus read is not implementable and the trust input reports unavailable with its reason; **CF-11** no component produces a suggestion, so the inbox is permanently empty in production — 4D builds the decision surface, not the initiative surface. Both were named rather than built around. `action-engine`, `nova-contracts` and `ws-gateway` untouched; `phase-4` and `main` untouched. See the [Gate Review](architecture-reviews/phase-4d-autonomy-engine-gate-review.md) and [`phase-4d.md`](../project-health/phase-4d.md) |
> | **4E** | `digital-twin-engine` extension + Digital Twin panel | **COMPLETE, VERIFIED, MERGED AND CLOSED — gate verdict GO with no conditions attached** (2026-09-15). Merged into `phase-4` on explicit user authorization as **`59bbeee3838290ab31b0627b64c6fc0b2bceb13b`**, a normal two-parent merge of PR #29 at head `df4606c`; `phase-4e` is preserved at `df4606c`, not deleted, and is an ancestor of `phase-4`, with **all ten 4E commits still reachable**. `main` is untouched at `7e273e6`, and **no `phase-4f` branch exists — no Phase 4F work has started.** ***Superseded 2026-09-16:*** that last clause was true when written and is preserved as written; Phase 4F has since started, and slices **4F.1** and **4F.2** are merged into `phase-4` (see the 4F row). Nothing else in this cell changes: 4E's own record, verdict and evidence are unaffected. *(That sentence read "**COMPLETE AND VERIFIED, NOT MERGED**" until the 2026-09-15 closure, and this cell read "Not started" until earlier the same day; both originals are preserved here per protocol §0.3.4.)* Built on `phase-4e`, head **`a044782360591e1d064e6dc053ce6926fc32c3e2`**, branched from `phase-4`'s merged head `f39fa6c` per master scope §16 rule 5 — eight commits, linear, zero merge commits, no force push, no rebase, no squash. **PR [#29](https://github.com/Tudor191/NOVA-PROJECT-BIBLE/pull/29)** (`phase-4e` → `phase-4`) is **open and NOT merged**; it was opened solely to obtain CI evidence, with merge authorization explicitly withheld. `phase-4` is untouched at `f39fa6c`, `main` at `7e273e6`, and **no `phase-4f` branch exists**. **AC-6 is MET at four tiers** — unit, integration, real-Postgres and browser — and it is 4E's only acceptance criterion. Bible Part 16's **nine remaining Digital Twin domains** (eleven total; `Communication Style` and `Preferences` shipped in 2D-D), four evidence states, six machine-readable reason codes, five REST routes behind one 1:1 D-6 gateway prefix, three additive tables in migration `0003` with **0 existing tables altered**, and a lazily-loaded panel. **0 new Event Bus subjects** — 118 registered at base, 118 at head; 4E is a *consumer* of three existing ones (`memory.long_term.created`, `memory.decision.recorded`, `perception.attention.observed`). `PUBLIC_TOPICS` **byte-identical**, **0 new dependencies**, and **0 files changed** in `autonomy-engine`, `personality-engine`, `action-engine`, `ws-gateway`, `perception-engine` or `agent-os`. **CI is 37/37 green at the exact head SHA**, zero failed and zero cancelled; **Playwright 17 passed / 1 skipped / 0 failed**, with all three AC-6 specs green in a real browser; `real_infra` **18 (digital-twin) + 4 (memory-engine)** passed, the `memory-engine` tier being new in 4E. **The two controls that matter most are types, not tests:** `DomainModel`'s validators make *"populated with zero evidence rows"* and *"a non-populated state with no reason code"* **unconstructible**, and enforce the ratified per-domain 4F floor, so a derivation bug raises rather than rendering a plausible number. **AC-6's "simulated multi-week gap" is simulated in the data, never in the clock** — real `created_at` values 118–37 days in the past, through the real repository write path into real PostgreSQL, on the real transactional outbox; no fake clock exists anywhere in the chain. The enabling fix was two lines in `PostgresMemoryRepository.create_long_term`, which had omitted `created_at`/`updated_at` and let the column defaults discard the caller's values — **no migration, and no Memory HTTP contract change**. **Two ratified decisions govern the scope:** **§20.1** (Option A plus a dedicated test driver; explicitly no `created_at` on `CreateMemoryRequest` or `long_term.write()`, no new endpoint, no fake clock) and **§20.2** (no new subject, no `PUBLIC_TOPICS` change, no `autonomy.*`, no `TrustMetric` surface, and CF-9/CF-10/CF-11 stay OPEN). **Three findings remain OPEN as explicitly non-blocking carry-forwards:** `nova_testkit`'s Postgres image drift (pre-existing), the D-6 prefix exposing six pre-4E operations that take a caller-supplied `user_id` (reported not fixed, pinned by two tests), and a stranded-outbox guard that does not yet watch the `memory` schema. **CF-9, CF-10 and CF-11 all remain OPEN — GO closes no carry-forward.** See the [Gate Review](architecture-reviews/phase-4e-digital-twin-gate-review.md) and [`phase-4e.md`](../project-health/phase-4e.md) |
> | **4F** | `nova-companion` senses, `cognitive-state-engine`, Autonomy Level 2 | **IN PROGRESS — 2 of 8 slices merged.** *(This cell read "Not started" until 2026-09-16; the original is preserved here per protocol §0.3.4, and was already stale from 4F.1's merge, which had no formal closure pass.)* **4F is one milestone with eight slices** (TDD 4F §18), so its Gate Review and Project Health record are written once, at 4F's closure — not per slice. **4F.1** (`cognitive-state-engine` domain + persistence) merged into `phase-4` as `4605d64950191bc08c4d1bc9d66a72ca8eaca030`, a normal two-parent merge of PR #30 at head `c0798a1`. **4F.2** (the `perception.workspace.observed` contract, structured intake and ingestion path) merged as `5b5ebfd03d6b41710fc2f427a78206236d1ff392`, a normal two-parent merge of PR #31 at head `cc0dcada`, CI **39/39 green**. Both branches are preserved and are ancestors of `phase-4`; `main` is untouched at `7e273e6`. **Exactly one new Event Bus subject** (118 → 119), `PUBLIC_TOPICS` **unchanged at 18**, and the new subject is provably internal — not public, not `ws-gateway`-subscribable, with `ws-gateway`'s bus allow-list narrowed from `perception.*` to the three browser-relevant perception subjects. `world-model-engine` production code is **unchanged**. **SLOC 45,280**, **4,720** below the 50,000 hard gate. **AC-7 and AC-8 are NOT yet met and are not claimed** — AC-7's honest end-to-end measurement is 4F.8's work (§20.1). **CF-9, CF-10 and CF-11 all remain OPEN.** Slices 4F.3–4F.8 have not been started. See the [4F.1 completion record](architecture-reviews/phase-4f1-cognitive-state-completion-record.md) and the [4F.2 completion record](architecture-reviews/phase-4f2-workspace-perception-completion-record.md), whose §11 carries the deferred-obligations ledger 4F's Gate Review inherits |
>
