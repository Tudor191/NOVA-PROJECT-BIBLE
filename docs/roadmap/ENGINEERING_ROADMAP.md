...
- **3B — Planning Persistence** (Postgres persistence, `GET`/`POST /v1/plans` API, `planning.task_graph.created`/`planning.decompose.request`, branch `phase-3b-planning-persistence`, PR #18) — Complete, fully verified locally and via real GitHub Actions (24/24 checks green, including `real-infra (planning-engine)`: 10/10 real-Postgres tests passed); **PR not yet merged**, pending explicit user approval. This closes TDD 3B's approved scope entirely except §6.1's `agent_os.task.completed` subscription, correctly deferred to Phase 3E. [Gate Review](architecture-reviews/phase-3b-planning-persistence-gate-review.md).
...
- **3E** (`agent-os` + first agents + supervisor) — **Implementation not started.** Architectural decision pass complete (2026-08-19): all four forks (3E-1 through 3E-4) **approved**, plus two additional open items (`nova-auth`/`PermissionGrant` scope, `priority`'s critical-path-position formula) **resolved** — [`14-3e-agent-os-research.md`](../design/phase-3/14-3e-agent-os-research.md) and `08-tdd-3e-agent-os.md` §11 carry the full record. Approving these architectural decisions is a separate step from approving the start of Phase 3E's own implementation PR, which has **not** been given. Gate Review structure prepared, explicitly not applicable yet — no code, CI, or test results exist to report: [Gate Review](architecture-reviews/phase-3e-agent-os-gate-review.md).
