...
> | **4D** | `autonomy-engine` Levels 0–1 + Autonomy panel | **Implemented and CI-verified — gate verdict CONDITIONAL-GO** (2026-09-13). Built on `phase-4d`, head `de6dbc9`, in four slices: the domain layer (Autonomy Levels, Policy Engine, Permission Matrix, Trust Engine, decision pipeline), the `autonomy` Postgres schema with doc 07's append-only `decision_log`, the `/v1/autonomy/*` REST surface fronted 1:1 by `api-gateway`, and the `autonomy/` panel. **Not merged.** PR **#27** was opened on 2026-09-13 **solely to obtain CI evidence**, with merge authorization explicitly withheld; head is now `f5f263f`. **36 checks, 28 success, and every check exercising 4D green**: `checks`, `real-infra (autonomy-engine)` **16/16 on two consecutive heads**, and `Playwright` with **both AC-5 specs passed in a real browser** against the real stack. **C-1, C-2 and C-3 are DISCHARGED.** **Two conditions remain open, neither an approved deferral**, so this verdict must still not be read as GO: **C-4** two TDD clauses need reconciling, and **C-5 (new)** `build-and-scan (ws-gateway)` fails Trivy on 3 CRITICAL + 9 HIGH pre-existing Debian CVEs — `git diff origin/phase-4..HEAD -- services/ws-gateway/` is **0 lines**, so it is not this milestone's, and fixing it is outside this branch's ratified scope, so it is reported rather than fixed. Eleven of twelve acceptance criteria fully Met; **AC-5 now Met at every tier including the browser**. All twelve of TDD §16's negative controls were demonstrated by removing the property and observing the suite fail. **Two ratified decisions govern the scope:** **D-4D-1** — 4D claims no `autonomy.*` Event Bus subject, both allow-lists stay empty, `PUBLIC_TOPICS` is byte-identical, and the AC-5 lifecycle runs over REST and the UI only; **D-4D-2** — **CF-9 stays OPEN**, `action-engine` keeps sole ownership of `IdentityConfidencePolicy`, and its fail-closed defaults are untouched. **Level 2 is defined and disabled** per decision D-1, which assigns enabling it to 4F. **Two new carry-forwards**: **CF-10** no read surface exists for 2D-D's `TrustMetric`, so TDD §5.3's Event Bus read is not implementable and the trust input reports unavailable with its reason; **CF-11** no component produces a suggestion, so the inbox is permanently empty in production — 4D builds the decision surface, not the initiative surface. Both were named rather than built around. `action-engine`, `nova-contracts` and `ws-gateway` untouched; `phase-4` and `main` untouched. See the [Gate Review](architecture-reviews/phase-4d-autonomy-engine-gate-review.md) and [`phase-4d.md`](../project-health/phase-4d.md) |
