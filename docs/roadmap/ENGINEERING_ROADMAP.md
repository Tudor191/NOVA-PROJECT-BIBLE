...
> | **4D** | `autonomy-engine` Levels 0–1 + Autonomy panel | **Implemented and locally verified — gate verdict CONDITIONAL-GO** (2026-09-13). Built on `phase-4d`, head `de6dbc9`, in four slices: the domain layer (Autonomy Levels, Policy Engine, Permission Matrix, Trust Engine, decision pipeline), the `autonomy` Postgres schema with doc 07's append-only `decision_log`, the `/v1/autonomy/*` REST surface fronted 1:1 by `api-gateway`, and the `autonomy/` panel. **Not merged, and no CI run exists at any SHA** — both workflows trigger only on `pull_request` or a push to `main`, and no pull request was opened, so field 15 of its health record reports an *absence* of CI evidence rather than a result. **Four open conditions, none an approved deferral**, so this verdict must not be read as GO: **C-1** no CI run · **C-2** the 16-test `real_infra` tier unexecuted (Docker unreachable locally) · **C-3** the AC-5 Playwright spec unexecuted · **C-4** two TDD clauses need reconciling. C-1 is the root of C-2 and C-3. Eleven of twelve acceptance criteria fully Met; **AC-5 Met at unit and integration tier**, unexecuted in a browser. All twelve of TDD §16's negative controls were demonstrated by removing the property and observing the suite fail. **Two ratified decisions govern the scope:** **D-4D-1** — 4D claims no `autonomy.*` Event Bus subject, both allow-lists stay empty, `PUBLIC_TOPICS` is byte-identical, and the AC-5 lifecycle runs over REST and the UI only; **D-4D-2** — **CF-9 stays OPEN**, `action-engine` keeps sole ownership of `IdentityConfidencePolicy`, and its fail-closed defaults are untouched. **Level 2 is defined and disabled** per decision D-1, which assigns enabling it to 4F. **Two new carry-forwards**: **CF-10** no read surface exists for 2D-D's `TrustMetric`, so TDD §5.3's Event Bus read is not implementable and the trust input reports unavailable with its reason; **CF-11** no component produces a suggestion, so the inbox is permanently empty in production — 4D builds the decision surface, not the initiative surface. Both were named rather than built around. `action-engine`, `nova-contracts` and `ws-gateway` untouched; `phase-4` and `main` untouched. See the [Gate Review](architecture-reviews/phase-4d-autonomy-engine-gate-review.md) and [`phase-4d.md`](../project-health/phase-4d.md) |
