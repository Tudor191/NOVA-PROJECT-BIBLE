...
> **Slice registration, 2026-09-26 (4F.7 ratification — TDD 4F §24),
> additively.** The milestone description above is preserved as written.
>
> - **4F remains one milestone**, and the Phase 4 set is still 4A–4F.
> - **One implementation slice is added before 4F.8**, by additive amendment
>   to TDD 4F §18: **4F.P, the production promotion slice** (RS-1c). It owns
>   `cognitive-state-engine`'s production promotion driver, thought ingestion
>   from existing Event Bus subjects, and `ProposedAction` authorship.
> - **4F.P's own TDD must ratify five things** before implementation: the
>   promotion policy, the thought-ingestion mapping, the authorship rule, the
>   CAS transition semantics and Layer 2 logical deduplication.
> - **4F.P is not started.** It has no TDD and no branch.
> - **4F.8 remains the final implementation slice.**
> - **4F.7 is strictly read-only** (RS-1b). Its slice TDD is
...
| [`08-tdd-4f4-identity-confidence-policy.md`](08-tdd-4f4-identity-confidence-policy.md) | **4F.4 slice design**, subordinate to TDD 4F: **CF-9's missing write surface** for `IdentityConfidencePolicy`, in the engine that already owns it. `action-engine` had enforced ADR-032's identity-confidence gate at pipeline stage 3 since Phase 3D and modelled the policy since then, but nothing in the repository could *create* the row it reads — so the threshold was 1.0 at every risk tier and every action was denied. Covers the single-resource `GET`/`PUT`/`DELETE /v1/action/identity-confidence-policy` surface, the two repository write methods, the `Settings.primary_user_id` the engine turned out not to have, the 0.75 writable ceiling and its source-text drift guard, plus scope, non-goals, acceptance criteria W-1…W-8, the real-Postgres evidence requirement, security boundaries, SLOC and deferred obligations. Declares **no new Event Bus subject, no `PUBLIC_TOPICS` change, no gateway change, no migration and no ORM model**. | **Written 2026-09-18 on the preparation branch `phase-4f4-tdd`; RATIFIED 2026-09-18 (§16).** All four decisions are recorded as **D-4F4-1 … D-4F4-4**: **risk tier *is* the capability class** for ADR-032 point 2, so the existing risk-tier-keyed schema is authoritative and no capability id or table is introduced; a **single-resource API** with server-derived `user_id`, 404 on absent `GET` and `DELETE` restoring fail-closed; **no audit trail**, with the gap opened as **L-15**; and **no production default threshold and no seed**, a deployment that installs nothing keeping the 1.0 fail-closed default. §16.5 adds the ceiling rule — a written threshold may not exceed **0.75** — enforced without importing `perception-engine` (ADR-004). §2.1 carries an additive correction: the TDD had asserted `Settings.primary_user_id` already existed in `action-engine` before anyone checked, and it did not. §10.2 records the D-1/D-2 discrepancy closure found by the final pre-Gate audit. **CF-9 stays OPEN** — conditions 2 and 5 cannot be met by a slice. *(Indexed 2026-09-20; this row was omitted when the document was written, a gap the 4F.5 TDD preparation found.)* |
| [`09-tdd-4f5-autonomy-level-2.md`](09-tdd-4f5-autonomy-level-2.md) | **4F.5 slice design**, subordinate to TDD 4F: Autonomy Level 2's five separable states and which three this slice owns (selectable, policy-permitted, executing — **not** the trigger, which is CF-11 and 4F.6's), the single dispatch point that makes AC-8's *"no code path differs"* literally true, `action.execute` acquiring its first producer as a **request/reply RPC** on an existing contract, and the disclosed retirement of 4D's control 8 in favour of the tighter *"publishes `action.execute` and nothing else"*. Declares **no new Event Bus subject, no `PUBLIC_TOPICS` change, no gateway change, no new REST route, no migration and no ORM model** — the `autonomy` schema has **zero CHECK constraints** and stores `policy.effect` and `decision_log.outcome` as `TEXT`, so a new policy effect and a newly-reachable `EXECUTE` outcome both persist unchanged. Records that **Digital Twin, Memory and Perception are read by nothing in this slice**. | **Written 2026-09-20 on the preparation branch `phase-4f5-tdd`; RATIFIED 2026-09-20 (§22).** All four decisions are recorded as **D-4F5-1 … D-4F5-4**: an affirmative **`PolicyEffect.AUTO_EXECUTE`** (never `ALLOW`), inert above `low`, with `DENY` taking precedence unconditionally and **absence leaving the fail-closed default untouched**; **execution fields on `DecisionRequest`**, required only at the dispatch branch, where absence yields a **suggestion rather than a guess** and no guessing table is introduced; a **bounded 15-second** `action.execute` timeout with **no retry and no second dispatch**, recorded as a **distinguishable** outcome in the existing decision log; and `permits_execution` becoming `level >= ASSISTED` as **eligibility, never permission** — dispatch additionally requires all seven preconditions of §22.4, each independently load-bearing. §11.2 carries the **fourteen ratified safety invariants** as required tests. *(This cell recorded the four questions as open until the ratification later the same day:* **A-4F5-1** how a policy may lower `requires_approval` without making an empty policy set permissive (the current `PolicyEvaluation.requires_approval` defaults to `False`, and `evaluate_gates` discards it — wiring it through as-is would be **fail-open**); **A-4F5-2** where `action.execute`'s mandatory `action_type`/`execution_target`/`verification_method` come from, given `DecisionRequest` carries none of them and `PermissionCategory`→`ActionType` is ten-to-two; **A-4F5-3** whether `decide()` awaits the RPC reply and with what timeout, given `action-engine`'s approval loop defaults to 300s; and **A-4F5-4** whether `permits_execution` becomes level-aware and what replaces `_forbid_execution` as §16 control 3's target. *.)* **CF-9, CF-10 and CF-11 all stay OPEN**, and this slice contributes closure evidence to none of them — it **must not claim to close CF-11**, because it creates no production caller of `decide()` |
| [`10-tdd-4f6-initiative-trigger.md`](10-tdd-4f6-initiative-trigger.md) | **4F.6 slice design**, subordinate to TDD 4F: the initiative trigger and Level 2's **Triggered** state — the first production caller of `decide()`, and CF-11's producer under §6.2. Revision 1 reported a blocking contradiction between TDD 4F §11.4, §12 and §13 that left no transport for the trigger; **D-4F-9** (its §14) resolves it additively with one **internal** `autonomy.decision.requested` subject. **RATIFIED 2026-09-22** — A-4F6-1, A-4F6-2a, A-4F6-2b, A-4F6-3 Layer 1, A-4F6-5, A-4F6-6 and A-4F6-7; **no blocking decisions remain**, and its §19 is the Implementation Contract. TTL and stale-trigger semantics stay **OPEN**; Layer 2 deduplication and lost-trigger auditability are **DEFERRED** |
| [`11-tdd-4f7-cognitive-state-panel.md`](11-tdd-4f7-cognitive-state-panel.md) | **4F.7 slice design**, subordinate to TDD 4F. It covers a **strictly read-only** `GET` surface under `/v1/cognitive-state`: Active Thoughts with every Part 6 field, Attention Layers, Focus (`score`, `signals_used`), normalized sensor state, and `proposed_action` shown **as a proposal only**. Around that surface it covers the `/v1/cognitive-state` `api-gateway` prefix, forwarded 1:1; the `cognitive-state/` panel; the engine's compose service and migrator entry; `perception-engine` publishing the **existing** `perception.sensor.health_changed` on its existing lifecycle transitions; and `cognitive-state-engine` subscribing to that one subject. Declares **no new Event Bus subject, no `PUBLIC_TOPICS` change, no write route, no promotion, and no thought creation or `ProposedAction` authoring**, all of which belong to **4F.P**. Proves only TDD 4F §4.1's *"visibly … in the panel"* row; OS-level revocation detection stays **OPEN** (RS-3b). | **Prepared 2026-09-26, after the 4F.7 ratification (TDD 4F §24, RS-1 … RS-11, all RATIFIED).** Two of its own decisions must be ratified before implementation: **A-4F7-1**, where normalized sensor state is held, and **A-4F7-2**, which `status` values `perception-engine` publishes. A-4F7-3 … A-4F7-6 carry recommended defaults. No branch has been created and nothing has been implemented. **CF-9, CF-10 and CF-11 stay OPEN** ***Updated 2026-09-26:*** the sentence about the two decisions was true when written and is preserved as written. **A-4F7-1 and A-4F7-2 are now RATIFIED** (TDD 4F.7 §28). Normalized sensor state lives in the durable current-state table `cognitive_state.sensor_state`, one record per sensor, with no history or audit semantics. The vocabulary is the existing `SensorState` lifecycle vocabulary (`uninitialized`, `initialized`, `running`, `paused`, `stopped`, `failed`), and unknown values are rejected. **The TDD is implementation-ready**; A-4F7-3 … A-4F7-6 stay PROPOSED with recommended defaults. **Implementation has not begun.** |
