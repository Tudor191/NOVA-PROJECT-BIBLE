...
| [`10-tdd-4f6-initiative-trigger.md`](10-tdd-4f6-initiative-trigger.md) | **4F.6 slice design**, subordinate to TDD 4F: the initiative trigger and Level 2's **Triggered** state — the first production caller of `decide()`, and CF-11's producer under §6.2. Revision 1 reported a blocking contradiction between TDD 4F §11.4, §12 and §13 that left no transport for the trigger; **A-4F6-1** resolves it with one **internal** `autonomy.decision.requested` subject and the additive amendment in its §14. **A-4F6-2 … A-4F6-6 remain PROPOSED**, and the trigger condition is blocked on **A-4F6-2a** |
