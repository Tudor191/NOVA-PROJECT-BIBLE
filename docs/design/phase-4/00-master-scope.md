...
| [`10-tdd-4f6-initiative-trigger.md`](10-tdd-4f6-initiative-trigger.md) | **4F.6 slice design**, subordinate to TDD 4F: the initiative trigger and Level 2's **Triggered** state — the first production caller of `decide()`, and CF-11's producer under §6.2. **PROPOSED, not ratified**: it records a blocking contradiction between TDD 4F §11.4, §12 and §13, which leaves no compliant transport for the trigger, and presents it as **A-4F6-1** rather than resolving it unilaterally |
