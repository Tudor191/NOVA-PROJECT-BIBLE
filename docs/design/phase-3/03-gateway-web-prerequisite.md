...
| Agent Activity panel | `3E` (`agent.*`/`agent_os.*` events, `GET /v1/agents`/`GET /v1/agents/{id}/activity`, both already named in doc 11 §2 but not yet implemented by any TDD in this package — flagged as a small, disclosed addition to `3E`'s own API surface, not built by this document). **Correction, 2026-08-29 (Phase 3E Gate Review), additive:** Phase 3E shipped and did **not** add those two endpoints. TDD 3E §4 gives `agent-os/kernel` a health-only surface (`/internal/health`, `/internal/readiness`, `/internal/metrics`) and no `/v1/...` REST surface at all, deliberately — the Kernel's work is Event-Bus- and internal-loop-driven. The `agent.*`/`agent_os.*` **events** this panel needs do exist and are published. The two REST endpoints remain unbuilt and are now an open `3-P` prerequisite with no owning TDD, not a "disclosed addition to `3E`'s own API surface". **Second correction, 2026-09-10 (Phase 4C.2g closure), additive — both halves of the row are now resolved, and one sentence above is withdrawn.** (1) **The claim that "the `agent.*`/`agent_os.*` events this panel needs do exist and are published" was wrong when written.** `agent.<instance_id>.<state>` has no payload in `nova-contracts` and appears in no component's `PUBLISHABLE_SUBJECTS`; the same Phase 3E Gate Review that added the note recorded its absence as DEV-2, and Phase 4 carries it as CF-8. `agent-os/kernel` publishes exactly one broadcast subject, `agent_os.task.completed`. The original sentence is left in place per the additive-correction rule and is superseded by this one. (2) **The REST prerequisite is discharged.** Phase 4C.2c built `GET /v1/agents`, `GET /v1/agents/{id}` and `GET /v1/agents/{id}/activity` on `agent-os/kernel` under decision D-4, fronted by `api-gateway`; Phase 4C.2e opened `agent_os.task.completed` to the browser; Phase 4C.2f built the panel. See Phase 4 master scope §9, §9.1 and §9.2. |
