...
| [Phase 3D](phase-3d.md) | **Conditional** — PR #13 open, CI 22/22 green, **not yet merged**; implementation satisfies TDD 3D §14 criteria 1-6 but not criterion 7 (deep `Capability.input_schema` validation at stage 5, required by the approved PR #12 §13 "final" acceptance criteria) — see the [Gate Review](../roadmap/architecture-reviews/phase-3d-action-engine-gate-review.md) | NR | NR | NR | 47 passed, 8 deselected (action-engine); capability-engine/nova-contracts unaffected, re-verified 61 passed / 86 passed | 97% (action-engine domain); capability-engine domain unchanged at 97% | 22/22 green, real GitHub Actions, reached green after a fix-forward commit (2 real bugs the real-infra job caught, invisible to local verification) | Fully real-infra confirmed via CI (8/8 real-Postgres tests, Docker build + Trivy scan) | One additive TDD 3D §14 correction note landed (2026-08-18, this documentation pass) disclosing the deep-validation gap; `phase-3d-research`/PR #12 itself deliberately not yet synced into canonical lineage, per the same sequencing precedent PR #11 established (research syncs after its implementation PR merges, not before) |
...
10. **Phase 2D-D, Phase 3A, both Phase 3B units, Phase 3C, Phase 3D** — **no Production SLOC or Total SLOC figure reported at all.** This is the reporting gap this whole `docs/project-health/` system exists to make visible and prevent from recurring silently.
