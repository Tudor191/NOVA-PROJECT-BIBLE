"""`clients.communication_client.CommunicationClient` (Phase 2D-D
docs/design/phase-2d/06-personal-companion.md Sec10.2, Fork D) -- a real
Event Bus round trip through `communication.session.lookup_by_user.
request` and `communication.intent.deliver.request`, mirroring
`communication-engine`'s own `test_digital_twin_client.py` convention: an
in-process stand-in for `communication-engine`'s own network position
*serves* both requests for real over the in-memory Event Bus, and this
engine's own (non-fake) `CommunicationClient`, constructed by `create_app`
with no override, calls them -- the wire contract itself is exercised, not
...
async def test_a_real_deliver_intent_call_reaches_a_real_intent_deliver_rpc_round_trip(
    ...
    async with app.router.lifespan_context(app):
        ...
        result = await app.state.communication_port.deliver_intent(
            session_id=session_id, content="Your build finished."
        )

        assert result is True
        assert len(received_requests) == 1
        assert received_requests[0].session_id == session_id
        assert received_requests[0].content == "Your build finished."
        assert received_requests[0].requesting_engine == "digital-twin-engine"
