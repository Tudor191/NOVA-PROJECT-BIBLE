"""Alembic environment for Memory Engine's `memory` Postgres schema.

Async-aware per SQLAlchemy 2.0's documented pattern (`connection.run_sync`) --
Memory Engine's own runtime is fully async (docs/architecture/03-backend-
...
from nova_memory_engine.repository.models import Base
...
def _do_run_migrations(connection: Connection) -> None:
    ...
    # (not `memory`) -- the `memory` schema doesn't exist yet on a fresh database
    # until migration 0001's own `CREATE SCHEMA` statement runs, so pointing
    # Alembic's own bookkeeping table at it would be a chicken-and-egg problem.
