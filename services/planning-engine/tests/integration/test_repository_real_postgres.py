...
async def test_insert_then_find_by_id_round_trips(
    ...
    await repository.insert(graph, outbox_event=_outbox_event(correlation_id=uuid4()))
...
async def test_append_nodes_mutates_in_place_and_recomputes_critical_path(
    ...
    updated = await repository.append_nodes(graph.id, [new_node], outbox_event_builder=_builder)
...
async def test_outbox_list_dispatch_ready_and_mark_dispatched_round_trip(
    ...
    graph = _graph()
    correlation_id = uuid4()
    await repository.insert(graph, outbox_event=_outbox_event(correlation_id=correlation_id))
