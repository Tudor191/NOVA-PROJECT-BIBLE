"""The transactional outbox dispatcher (docs/design/phase-2a/
00-ai-model-orchestration-engine.md §17). This engine owns no graph (§4), so
there is no two-phase saga to run -- a `usage_record` write and its outbox
row commit together in one Postgres transaction; this dispatcher's only job is
...
Phase 1 engine's outbox dispatcher uses.
"""
