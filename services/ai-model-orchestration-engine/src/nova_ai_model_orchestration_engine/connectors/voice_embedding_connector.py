"""The default, zero-budget voice-embedding connector (docs/design/phase-2d/
03-perception-engine.md §0.2). Talks to a local ECAPA-TDNN-class speaker-
embedding server over a small custom `POST /v1/audio/embed` endpoint -- the
same "lazily imported `httpx`, never required just to import this module"
convention as every other connector.

Every other modality raises `NotSupportedError` -- voice embedding only.
Distinct from `OllamaConnector.embed` (text embedding): this connector never
touches text.
"""
...
__all__ = ["VoiceEmbeddingConnector"]


class VoiceEmbeddingConnector:
    connector_type = "voice_embedding"
    ...
    async def embed_voice(self, request: VoiceEmbedRequest) -> VoiceEmbedResult:
        ...
        response = await client.post("/v1/audio/embed", files=files)
        response.raise_for_status()
        body = response.json()
        embedding = body.get("embedding", [])
        return VoiceEmbedResult(
            embedding=embedding, structural_confidence=1.0 if embedding else 0.0
        )
