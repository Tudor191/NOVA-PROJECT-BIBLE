"""Alembic environment for the AI Model Orchestration Engine's
`model_orchestration` Postgres schema.
...
from nova_ai_model_orchestration_engine.repository.models import Base
...
def run_migrations_offline() -> None:
    context.configure(
        ...
        version_table="alembic_version_model_orchestration",
    )
...
def _do_run_migrations(connection: Connection) -> None:
    ...
    # (not `model_orchestration`) -- that schema doesn't exist yet on a fresh
    # database until migration 0001's own `CREATE SCHEMA` statement runs, so
    # pointing Alembic's own bookkeeping table at it would be a
