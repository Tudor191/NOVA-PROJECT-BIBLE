...
**What this module deliberately does not have.** No execution verb, no action
type, no capability reference, no way to express "do this". TDD 4F §6.2 is
explicit that this engine *proposes* and never acts, and the cheapest way to
keep that true is for the vocabulary to be unable to say otherwise. The
`DecisionRequest` this engine will eventually produce (4F.6) is
`autonomy-engine`'s type, not one defined here.
