"""Alembic environment for World Model Engine's `world_model` Postgres schema.

Async-aware per SQLAlchemy 2.0's documented pattern (`connection.run_sync`) --
World Model Engine's own runtime is fully async (docs/architecture/03-backend-
...
from nova_world_model_engine.repository.models import Base
...
def run_migrations_offline() -> None:
    context.configure(
        ...
        version_table="alembic_version_world_model",
    )
...
def _do_run_migrations(connection: Connection) -> None:
    ...
    # (not `world_model`) -- the `world_model` schema doesn't exist yet on a
    # fresh database until migration 0001's own `CREATE SCHEMA` statement runs,
    # so pointing Alembic's own bookkeeping table at it would be a
