"""End-to-end proof of the qa-agent slice: a real Kernel `create_app()`
(real Scheduler, real `RegistryClient` RPC, real `InprocessExecutionBackend`)
dispatches through the real `agents/qa-agent/src/handler.py` Handler --
mirrors `test_full_loop_research_agent.py`'s own structure and stand-in
conventions exactly, using a `FakeActionPort` in place of research-agent's
own `FakeModelGatewayPort` (qa-agent's own `execute()` uses `action_port`,
not `model_gateway`, mirrors `coding-agent`'s own full-loop test).

No Supervisors app is involved here, unlike `coding-agent`'s own full-loop
test -- `qa-agent`'s manifest declares no `peer_reviewer_category` (TDD 3E
§9's own agent table gives it no reviewer/reviewee role), so
`dispatch_task_node()` never calls `SupervisorPort.record_peer_review()`
for a successful qa-agent dispatch, and the default `SupervisorClient`
(unused in the success path) needs no real or stand-in server."""
...
class _UnusedModelGatewayPort:
    """qa-agent's own `execute()` never calls `model_gateway` (it uses
    `action_port` instead, see `agents/qa-agent/src/handler.py`) -- this
    stand-in exists only to satisfy `InprocessExecutionBackend`'s own
    ...
    async def generate(self, request: GenerateRequestPayload) -> GenerateReplyPayload:
        raise AssertionError("qa-agent's execute() does not call model_gateway")


def _package_snapshot() -> AgentPackageSnapshot:
    """Matches, field-for-field, what Registry's own real install pipeline
    produces for `agents/qa-agent/agent.yaml`, independently proven by
    `agent-os/registry`'s own `test_real_qa_agent_installs.py`."""
...
async def test_full_loop_dispatches_the_real_qa_agent(
    ...
    action_port = _FakeActionPort(
        reply=ActionResultPayload(
            action_id=uuid4(),
            status="completed",
            result={"exit_code": 0, "stdout": "7 passed", "stderr": ""},
            error=None,
        )
    )
    ...
    async with kernel_app.router.lifespan_context(kernel_app):
        ...
        assert completed_payload.result["output"]["exit_code"] == 0
